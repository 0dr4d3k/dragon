...
// GAP Role Callbacks
static gapPeripheralRoleCBs_t hidDev_PeripheralCBs =
{
...
uint16 HidDev_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  {
    // Start the Device
    VOID GAPPeripheralRole_StartDevice( &hidDev_PeripheralCBs );
...
void HidDev_Close( void )
{
  ...
  {
    param = FALSE;
    GAPPeripheralRole_SetParameter( GAPROLE_ADVERT_ENABLED, sizeof( uint8 ), &param );
  }
}
...
static void hidDevGapStateCB( gaprole_States_t newState )
{
  ...
  if ( newState == GAPROLE_CONNECTED )
  {
    ...
    GAPPeripheralRole_GetParameter( GAPROLE_CONNHANDLE, &gapConnHandle );
...
static void hidDevSendReport( uint8 id, uint8 type, uint8 len, uint8 *pData )
{
  ...
  if ( (pRpt = hidDevRptById(id, type)) != NULL )
  {
    ...
    if ( value & GATT_CLIENT_CFG_NOTIFY )
    {
      ...
      if ( updateConnParams )
      {
        GAPPeripheralRole_SetParameter( GAPROLE_PARAM_UPDATE_REQ, sizeof( uint8 ), &updateConnParams );
...
static void hidDevHighAdvertising( void )
{
  ...
  VOID GAPPeripheralRole_SetParameter( GAPROLE_ADV_FILTER_POLICY, sizeof( uint8 ), &param );
...
static void hidDevLowAdvertising( void )
{
  ...
  param = TRUE;
  VOID GAPPeripheralRole_SetParameter( GAPROLE_ADVERT_ENABLED, sizeof( uint8 ), &param );
}
