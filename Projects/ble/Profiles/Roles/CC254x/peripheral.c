...
static gapPeripheralRoleCBs_t *pGapRoles_AppCBs = NULL;
 ...
 * LOCAL FUNCTIONS
 */
static void gapPeripheralRole_ProcessOSALMsg( osal_event_hdr_t *pMsg );
static void gapPeripheralRole_ProcessGattEvent(gattMsgEvent_t *pMsg);
static void gapPeripheralRole_ProcessGAPMsg( gapEventHdr_t *pMsg );
static void gapPeripheralRole_SetupGAP( void );
static void gapPeripheralRole_HandleParamUpdateNoSuccess( void );
static void gapPeripheralRole_StartConnUpdate( uint8 handleFailure );
 ...
 */
bStatus_t GAPPeripheralRole_SetParameter( uint16 param, uint8 len, void *pValue )
{
      ...
      case GAPROLE_PARAM_UPDATE_REQ:
        {
          ...
          if ( len == sizeof ( uint8 ) && (req == TRUE) )
          {
            ...
            {             
              ...
              gapPeripheralRole_StartConnUpdate( GAPROLE_NO_ACTION );
 ...
 * Public function defined in peripheral.h.
 */
bStatus_t GAPPeripheralRole_GetParameter( uint16 param, void *pValue )
{
  bStatus_t ret = SUCCESS;
  switch ( param )
  {
 ...
 * Public function defined in peripheral.h.
 */
bStatus_t GAPPeripheralRole_StartDevice( gapPeripheralRoleCBs_t *pAppCallbacks )
{ 
  // Clear all of the Application callbacks
  if ( pAppCallbacks )
  {
    pGapRoles_AppCBs = pAppCallbacks;
  }
  
  // Start the GAP
  gapPeripheralRole_SetupGAP();
 ...
 * Public function defined in peripheral.h.
 */
void GAPPeripheralRole_RegisterAppCBs(gapRolesParamUpdateCB_t *pParamUpdateCB)
{
 ...
 * Public function defined in peripheral.h.
 */
bStatus_t GAPPeripheralRole_TerminateConnection( void )
{
 ...
 */
void GAPPeripheralRole_Init( uint8 task_id )
{
...
uint16 GAPRole_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  if ( events & SYS_EVENT_MSG )
  {
    ...
    {
      gapPeripheralRole_ProcessOSALMsg( (osal_event_hdr_t *)pMsg );
  ...
  if ( events & START_ADVERTISING_EVT )
  {
    if ( gapRole_AdvEnabled || gapRole_AdvNonConnEnabled )
    {
      ...
      if ( GAP_MakeDiscoverable( gapRole_TaskID, &params ) != SUCCESS )
      {
        ...
        if ( pGapRoles_AppCBs && pGapRoles_AppCBs->pfnStateChange )
        {
          pGapRoles_AppCBs->pfnStateChange( gapRole_state );
        }
      }
    }
  ...
  if ( events & CONN_PARAM_TIMEOUT_EVT )
  {
    ...
    gapPeripheralRole_HandleParamUpdateNoSuccess();
...
/*********************************************************************
 * @fn      gapPeripheralRole_ProcessOSALMsg
 *
...
static void gapPeripheralRole_ProcessOSALMsg( osal_event_hdr_t *pMsg )
{
  switch ( pMsg->event )
  {
    case HCI_GAP_EVENT_EVENT:
      if ( pMsg->status == HCI_COMMAND_COMPLETE_EVENT_CODE )
      {
        ...
        if ( pPkt->cmdOpcode == HCI_READ_RSSI )
        {
          ...
          {
            // Report RSSI to app
            if ( pGapRoles_AppCBs && pGapRoles_AppCBs->pfnRssiRead )
            {
              pGapRoles_AppCBs->pfnRssiRead( rssi );
            }
          }
        }
      }
    ...
    case GATT_MSG_EVENT:
      // Process GATT event
      gapPeripheralRole_ProcessGattEvent((gattMsgEvent_t *)pMsg);
      break;
              
    case GAP_MSG_EVENT:
      gapPeripheralRole_ProcessGAPMsg( (gapEventHdr_t *)pMsg );
      break;
...
/*********************************************************************
 * @fn      gapPeripheralRole_ProcessGattEvent
 *
...
/*********************************************************************
 * @fn      gapPeripheralRole_ProcessGAPMsg
 *
 * @brief   Process an incoming task message.
 *
 * @param   pMsg - message to process
...
/*********************************************************************
 * @fn      gapPeripheralRole_HandleParamUpdateNoSuccess
 *
...
static void gapPeripheralRole_HandleParamUpdateNoSuccess( void )
{
  ...
  switch ( paramUpdateNoSuccessOption )
  {
    case GAPROLE_RESEND_PARAM_UPDATE:
      GAPPeriperalRole_SendUpdateParam(gapRole_MinConnInterval, 
...
/********************************************************************
 * @fn          gapPeripheralRole_StartConnUpdate
 *
...
/********************************************************************
 * @fn          GAPPeriperalRole_SendUpdateParam
 *
 ...
 */
bStatus_t GAPPeriperalRole_SendUpdateParam( uint16 minConnInterval, uint16 maxConnInterval,
...
{    
       ...
       ( connTimeout     < MAX_TIMEOUT_MULTIPLIER     ) )
  {
    ...
    // Start connection update procedure
    gapPeripheralRole_StartConnUpdate( handleFailure );
