...
int main(void)
{
     ...
     */
#if (chip==2541 || chip==2543 || chip==2545)
    P1SEL &= ~P1_PIN;           // Select pin to function as General Purpose I/O.
    P1DIR &= ~P1_PIN;           // Select direction as input.
    P1INP &= ~P1_PIN;           // Pull up.
//    PPULL &= ~(PPULL_PDUP0H | PPULL_PDUP0L);     // P0_[7:4] and P0_[3:0] pull up.
    ...
    P1IFG = ~P1_PIN;            // Clear status flag for pin.
    P1IF = 0;                   // Clear CPU interrupt status flag for P0.

    // Set individual interrupt enable bit in the peripherals SFR.
    P1IEN |= P1_PIN;                              // Enable interrupt from pin.


#if (chip==2541 || chip==2543 || chip==2545)
    PICTL |= (PICTL_P1ICONH);    // Falling edge, P1_[7:4]
#elif (chip==2544)
