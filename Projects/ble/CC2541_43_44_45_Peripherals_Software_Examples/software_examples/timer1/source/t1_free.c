...
//    P2DIR |= P2DIR_PRIP0_T1_0_1;
    ...
    // Configure P0_1 as GPIO output for LED1.
    P0SEL &= (BIT0 | BIT1 | BIT2) ;      // GPIO.
    P0DIR |= BIT0 | BIT1 | BIT2 ;      // Output.
//    P0SEL &= BIT1 ;      // GPIO.
 //   P0DIR |= BIT1 ;      // Output.
//    P0_1 = BIT1;           // LED1 off.
//    P0_2 = BIT2;           // LED1 off.
//    P0 = 0x07;
    ...
    */
    ...
    T1CCTL1 &= ~T1CCTLn_IM;     // Enable interrupt on channel 1.
    ...
    T1OVFIM = 0;                // Enable overflow interrupt.
...
// Configure Timer 1 setting
    ...
    T1CC0H = 0x00;
          ...
          //Timer 1 Channel 2 duty cycle
          T1CC2L = 0x7F;  // PWM duty cycle
          T1CC2H = 0x00;
          T1CCTL2 = (T1CCTL2 & ~0x23) | 0x1C; // PWM output

          //Timer 1 Channel 3 duty cycle
          T1CC3L = 0x7F;  // PWM duty cycle
          T1CC3H = 0x00;
          T1CCTL3 = (T1CCTL3 & ~0x23) | 0x1C; // PWM output

          //Timer 1 Channel 4 duty cycle
          T1CC4L = 0x7F;  // PWM duty cycle
          T1CC4H = 0x00;
          T1CCTL4 = (T1CCTL4 & ~0x23) | 0x1C; // PWM output
