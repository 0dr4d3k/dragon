...
#undef UTXxIE
#undef UTXxIF
// oJo, Dragonfly justo tiene la UART0 en Alt-2(P1)  
#if    (HAL_UART_DMA == 1)
#define UxCSR                      U0CSR
#define UxUCR                      U0UCR
...
// Falling edge ISR on P1.0-3 pins. 
// oJo interfiere con hal_interrupts ISR port P1!!!
// poner DMA_PM FALSE para deshabilitar el ahorro de energia en la UART a traves de los pines CTS y RTS
...
//oJo, lo he modificado a static
static void HalUART_DMAIsrDMA(void);
...
static void HalUARTInitDMA(void)
{
  ...
  P2DIR &= ~P2DIR_PRIPO;             // Delete old values
...
static void HalUARTPollDMA(void)
{
  ...
  {
//-- Cagate con el que hizo esto -->   dmaCfg.uartCB(HAL_UART_DMA-1, evt);
#if  (HAL_UART_DMA == 2) || (HAL_UART_DMA == 3)
    dmaCfg.uartCB(HAL_UART_PORT_1, evt);
#elif (HAL_UART_DMA == 1)    
    dmaCfg.uartCB(HAL_UART_PORT_0, evt);
#endif
  }
