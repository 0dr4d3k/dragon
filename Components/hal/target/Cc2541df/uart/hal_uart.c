...
#if HAL_UART_DMA
#include "_hal_uart_dma.h"
#endif
#if HAL_UART_ISR
#include "_hal_uart_isr.h"
#endif
#if HAL_UART_USB
#include "_hal_uart_usb.h"
#endif
 ...
 * @fn      HalUARTPrintf        
 *
 * @brief   Write a formated buffer to the UART.
 *
 * @param   *format_string - pointer to format string
 *          arguments
 *          [..]
 *
 * @return  none
 *****************************************************************************/
void HalUARTPrintf(char *format_string, ...)
{
#if defined DEBUG_OVER_UART & DEBUG_OVER_UART == TRUE
  char buff[64];
  uint8 length;

  va_list args;
  va_start(args, format_string);
  length = vsprintf(buff, format_string, args);
  va_end(args);

  HalUARTWrite(HAL_UART_PORT_0, (uint8 *)buff, length);
#endif
}
