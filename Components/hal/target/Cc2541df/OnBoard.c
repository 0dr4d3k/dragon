...
uint8 OnBoard_SendKeys( uint8 keys, uint8 state )
{
  ...
  if ( registeredKeysTaskID != NO_TASK_ID )
  {
    ...
    if ( msgPtr )
    {
      //oJo, meter aqui la demultiplexacion de la interrupcion en eventos segun el tipo de board
...
//  shift = (OnboardKeyIntEnable == HAL_KEY_INTERRUPT_ENABLE) ? false : ((keys & HAL_KEY_SW_4) ? true : false);

// oJo, esto no lo utilizamos en Dragonfly, eliminarlo si eso
